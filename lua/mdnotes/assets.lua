...
function M.check_assets_path()
    ...
    if vim.fn.isdirectory(vim.fs.joinpath(mdnotes_cwd, mdnotes_assets_path)) == 0 then
        vim.notify("Mdn: Assets path '".. mdnotes_assets_path .. "' doesn't exist - change path or create it", vim.log.levels.ERROR)
        return false
    end
...
local function move_delete_unused(move_or_delete)
    if M.check_assets_path() == false then return end

    local used_assets = M.get_used_assets(true)
    local mdnotes_config = require('mdnotes').config
