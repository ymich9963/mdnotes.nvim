...
local function get_current_header(buf_sections)
    local cur_buf_num = vim.api.nvim_get_current_buf()
    local cur_lnum = vim.fn.line(".")

    for _, v in ipairs(buf_sections) do
...
function M.goto_next()
    local buf_sections = require('mdnotes.toc').buf_sections
    local _, index, cur_buf_num = get_current_header(buf_sections)
